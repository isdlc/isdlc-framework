...
if [[ "$TOUR_CHOICE" =~ ^[Yy] ]]; then
    ...
    echo ""
    echo "  iSDLC is an AI Development Governance Harness — it wraps your"
    echo "  project with structure, enforcement, and context management for"
    echo "  enterprise AI-assisted software engineering."
    echo ""
    echo -e "${CYAN}━━━ What you get ━━━${NC}"
    echo ""
    echo -e "  ${GREEN}▸${NC} Constitutional governance       Living rulebook every agent must follow"
    echo -e "  ${GREEN}▸${NC} Spec-driven artifacts           Versioned markdown + JSON checked into your repo"
    echo -e "  ${GREEN}▸${NC} Deterministic enforcement       Runtime hooks that actually block drift"
    echo -e "  ${GREEN}▸${NC} Structured task management      tasks.md with dependency graphs + traceability"
    echo -e "  ${GREEN}▸${NC} Managed context                 Constitution + skills + session cache on every call"
    echo -e "  ${GREEN}▸${NC} Semantic code search (optional) Local embeddings, no data leaves your machine"
    echo -e "  ${GREEN}▸${NC} Hackable by design              Agents, skills, commands, hooks — all markdown / JS"
    echo -e "  ${GREEN}▸${NC} Multi-provider                  Claude Code, Codex, Anthropic API, local Ollama"
    echo ""
    echo -e "${CYAN}━━━ Core commands ━━━${NC}"
    echo ""
    echo -e "  ${GREEN}/discover${NC}  ${GREEN}/add${NC}  ${GREEN}/analyze${NC}  ${GREEN}/build${NC}  ${GREEN}/isdlc${NC}  ${GREEN}/provider${NC}  ${GREEN}/tour${NC}"
    echo ""
    echo -e "  Full documentation:     ${GREEN}https://github.com/vihang-hub/isdlc-framework${NC}"
    echo -e "  Interactive walkthrough: run ${GREEN}/tour${NC} in Claude Code"
    echo ""
else
    echo ""
...
STEP=$((STEP + 1))
echo -e "  ${STEP}. Run ${GREEN}/tour${NC} anytime for the interactive walkthrough"
echo ""
if [ "$IS_EXISTING_PROJECT" = true ]; then
