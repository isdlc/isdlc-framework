...
function showHelp() {
  ...
  logger.log('  --provider <name>       Set LLM provider (claude, codex, cursor, windsurf)');
    ...
    if (arg.startsWith('--')) {
      ...
      switch (option) {
        ...
        case 'no-search-setup':
          ...
          break;
        case 'provider':
          result.options.provider = args[++i] || null;
          break;
        case 'user':
...
/**
 * Detect the active LLM provider using a priority chain.
 * Priority: CLI flag > providers.yaml > autoDetectProvider() > 'claude'
 *
 * Traces: REQ-0137 FR-001
 *
 * @param {object} options - Parsed CLI options
 * @param {string} projectRoot - Project root directory
 * @returns {Promise<string>} Provider name
 */
export async function detectProvider(options, projectRoot) {
  try {
    ...
    // 2. Load providers.yaml config (dynamic import to avoid breaking existing installs)
    try {
      const { loadProvidersConfig, hasProvidersConfig } = await import('../src/core/providers/config.js');
      // Only use config tier if an explicit providers.yaml exists in the project
      if (hasProvidersConfig(projectRoot)) {
        const config = loadProvidersConfig(projectRoot);
      ...
      // Config module unavailable — continue to next tier
    }

    // 3. Auto-detect provider
    try {
      const { autoDetectProvider } = await import('../src/core/providers/routing.js');
      const { loadProvidersConfig } = await import('../src/core/providers/config.js');
      const config = loadProvidersConfig(projectRoot);
      const result = await autoDetectProvider(config);
      if (result?.provider) {
        // Map 'anthropic' to 'claude' for instruction file naming
        return result.provider === 'anthropic' ? 'claude' : result.provider;
      }
    } catch {
      // Auto-detect unavailable — fall through
    }

    // 4. Fail-safe default (Article X)
    return 'claude';
  } catch {
    return 'claude';
  }
...
/**
 * Generate and write provider instruction file after install/update.
 * Fail-open: logs warning on failure, does not throw.
 *
 * @param {string} providerName - Provider name
 * @param {string} projectRoot - Project root directory
 * @param {object} options - CLI options (dryRun, etc.)
 */
async function generateProviderInstructions(providerName, projectRoot, options) {
  try {
    const { generateInstructions, getInstructionPath } = await import('../src/core/orchestration/instruction-generator.js');
    const { existsSync } = await import('node:fs');
    const outputPath = getInstructionPath(providerName, projectRoot);

    // For Claude provider, the installer already creates CLAUDE.md from a template.
    // Do not overwrite it — the installer's template includes issue tracker config
    // and other project-specific content. Only generate for non-Claude providers,
    // or if the instruction file does not already exist.
    if (existsSync(outputPath)) {
      return;
    }

    const projectName = path.basename(projectRoot);
    const result = generateInstructions(providerName, { projectName, projectRoot });

    if (options.dryRun) {
      logger.log(`[dry-run] Would generate ${result.fileName} for ${providerName} provider`);
    } else {
      const { writeFileSync } = await import('node:fs');
      writeFileSync(outputPath, result.content, 'utf8');
      logger.log(`Generated ${result.fileName} for ${providerName} provider`);
    }
  } catch (err) {
    // Fail-open: instruction generation is non-critical
    logger.log(`Note: Instruction file generation skipped (${err.message})`);
  }
}
  ...
  try {
    switch (command) {
      case 'init': {
        ...
        // REQ-0136/REQ-0137: Generate provider instruction file after install
        const providerName = await detectProvider(options, projectRoot);
        await generateProviderInstructions(providerName, projectRoot, options);
        break;
      }

      case 'update': {
        const updateRoot = process.cwd();
        await runUpdate(options);
        // REQ-0136/REQ-0137: Regenerate provider instruction file after update
        const updateProvider = await detectProvider(options, updateRoot);
        await generateProviderInstructions(updateProvider, updateRoot, options);
        break;
      }
...
export { parseArgs };
export default { run, parseArgs, detectProvider };
