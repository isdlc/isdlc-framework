 ...
 * @param {number} [config.workloadSize] - REQ-GH-248 FR-006: number of texts to embed (workload floor)
...
export async function createJinaCodeAdapter(config = {}) {
  ...
  const resolved = resolveConfig(
    ...
    config._platformEnv,
    {
      // REQ-GH-248 FR-006: thread workloadSize through so the auto
      // parallelism decision knows how many chunks are in the batch.
      workloadSize: config.workloadSize,
    },
  );
...
function createPooledAdapter(config, resolved, pipelineFactory) {
  ...
  // REQ-GH-248 FR-007 / AC-007-01..03: pass the calibrated per-worker
  // memory value through to pool construction. Prior to this fix, the
  // adapter silently re-clamped against the hardcoded constants
  // (WORKER_MEMORY_ESTIMATE_GB[device]), defeating the calibration cache.
  //
  // Preference order:
  //   1. resolved.perWorkerMemGB from resolveConfig (calibrated or
  //      hardcoded fallback already chosen by device-detector)
  //   2. hardcoded WORKER_MEMORY_ESTIMATE_GB fallback (Article X)
  const perWorkerMemGB =
    (Number.isFinite(resolved.perWorkerMemGB) && resolved.perWorkerMemGB > 0)
      ? resolved.perWorkerMemGB
      : (WORKER_MEMORY_ESTIMATE_GB[resolved.device] || 3);
  ...
  const pool = createPool(workerPath(), {
    ...
    perWorkerMemGB,
    // REQ-GH-248 FR-006: the pool also receives the workload size so its
    // own resolvePoolSize() path respects the workload floor when
    // poolSize is 'auto'.
    workloadSize: config.workloadSize,
  });
