...
// ===========================================================================
// REQ-GH-248 FR-006 / FR-008: Workload-aware resolvePoolSize + dedup
// ===========================================================================

describe('REQ-GH-248 FR-006/FR-008: resolvePoolSize workload-aware + dedup', () => {
  const GB = 1024 ** 3;

  it(
    '[P1] REQ-GH-248 FR-006 AC-006-01 TC-006-05: Given poolSize="auto", cpuCount=10, totalMem=24GB, perWorker=3, workloadSize=4, batchSize=32, When resolvePoolSize runs, Then the result is 1 (workload floor ceil(4/32/2)=1)',
    () => {
      const result = resolvePoolSize(
        'auto',
        () => 10,
        () => 24 * GB,
        3,
  ...
  it(
    '[P0] REQ-GH-248 FR-008 AC-008-01 TC-008-02: Given identical inputs, When both autoParallelism math (via computeEffectiveParallelism) and resolvePoolSize are called, Then they produce the SAME result',
    () => {
      // Shared matrix — same as computeEffectiveParallelism tests above
      const cases = [
      ...
      for (const c of cases) {
        const direct = computeEffectiveParallelism(c);
        assert.equal(direct, c.expected, `direct call got ${direct}`);
      }
    }
  );

  it(
    '[P0] REQ-GH-248 FR-008 AC-008-02 TC-008-03: worker-pool imports the dedup\'d constants from device-detector.js (single source of truth)',
    () => {
      // Imports at the top of this file — verify they are defined, positive
      // numbers, and that the values are what device-detector exports. This
      // pins the single source of truth and catches accidental duplication.
      assert.equal(typeof PARALLELISM_HARD_CAP, 'number');
      assert.equal(typeof SYSTEM_RESERVED_GB, 'number');
      assert.equal(typeof MIN_BATCHES_PER_WORKER, 'number');
      assert.ok(PARALLELISM_HARD_CAP > 0);
      assert.ok(SYSTEM_RESERVED_GB > 0);
      assert.ok(MIN_BATCHES_PER_WORKER > 0);
      // WORKER_MEMORY_ESTIMATE_GB is an object with device keys
      assert.ok(typeof WORKER_MEMORY_ESTIMATE_GB === 'object');
      assert.ok(Number.isFinite(WORKER_MEMORY_ESTIMATE_GB.coreml));
    }
  );

  it(
    '[P1] REQ-GH-248 FR-006: resolvePoolSize with no workloadSize behaves like the pre-fix version (no workload floor applied)',
    () => {
      // 64GB, 8 cpus, perWorker 3 → maxByMem=floor(44.8/3)=14, cpu=7, cap=4 → 4
      const result = resolvePoolSize('auto', () => 8, () => 64 * GB, 3);
      assert.equal(result, 4);
    }
  );

  it(
    '[P2] REQ-GH-248 FR-006: resolvePoolSize with workloadSize=0 ignores the workload floor',
    () => {
      const result = resolvePoolSize('auto', () => 8, () => 64 * GB, 3, 0, 32);
      assert.equal(result, 4);
    }
  );

  it(
    '[P1] REQ-GH-248 FR-006: createWorkerPool accepts workloadSize option and forwards to resolvePoolSize',
    () => {
      // We don't need to actually spawn workers — just verify the option
      // is plumbed through by checking the effective pool size via stats().
      // Use the MockWorker class from the outer suite.
      class TinyMockWorker extends EventEmitter {
        constructor() { super(); this.terminated = false; }
        postMessage() { /* no-op */ }
        terminate() { this.terminated = true; return Promise.resolve(0); }
      }
      const pool = createWorkerPool('mock.js', {
        poolSize: 'auto',
        _cpuCountFn: () => 10,
        _totalMemFn: () => 24 * GB,
        _WorkerClass: TinyMockWorker,
        perWorkerMemGB: 3,
        workloadSize: 4,
        batchSize: 32,
      });
      // Workload floor ceil(4/32/2) = 1 → poolSize = 1
      assert.equal(pool.stats().poolSize, 1);
