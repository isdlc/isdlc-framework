...
export async function embed(texts, config, options = {}) {
  ...
  // REQ-GH-248 FR-006: thread the workloadSize (= texts.length) through to
  // the adapter so resolveConfig / resolvePoolSize can apply the workload
  // floor to auto-parallelism. We set it on a shallow copy of config so
  // the caller's object is not mutated.
  const adapter = await resolveAdapter({ ...config, workloadSize: texts.length });
...
async function resolveAdapter(config) {
  ...
  switch (config.provider) {
    case 'jina-code': {
      ...
      // REQ-GH-248 FR-006: workload floor threading
      if (config.workloadSize !== undefined) adapterConfig.workloadSize = config.workloadSize;
