...
import {
  ...
  computeWorkloadFloor,
  computeEffectiveParallelism,
} from './device-detector.js';
import { computeFingerprint, CACHE_FILENAME } from './memory-calibrator.js';
...
// ===========================================================================
// REQ-GH-239: FR-003 / FR-009 / NFR-001 — Calibration integration
// ===========================================================================

/**
 * Create a sandbox project root with a .isdlc directory. Returns the path
 * and a cleanup function. Tests that touch real fs use this so the real
 * framework state.json is never disturbed.
 */
function makeSandboxProjectRoot(prefix = 'device-detector-calibration-') {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), prefix));
  fs.mkdirSync(path.join(root, '.isdlc'), { recursive: true });
  return {
    root,
    cleanup() {
...
function writeCalibrationCache(projectRoot, config, overrides = {}) {
  ...
  fs.writeFileSync(
    path.join(projectRoot, '.isdlc', CACHE_FILENAME),
...
describe('REQ-GH-239 FR-003: resolvePerWorkerMemGB()', () => {

  it('[P0] AC: cache hit returns calibrated value with source=calibrated and measuredAt populated', () => {
    const sb = makeSandboxProjectRoot();
    try {
      const config = { device: 'coreml', dtype: 'fp16', model: 'jina-code-v2' };
      const cached = writeCalibrationCache(sb.root, config);
      const result = resolvePerWorkerMemGB(config, sb.root);
      assert.equal(result.source, 'calibrated');
      assert.equal(result.value, cached.perWorkerMemGB);
      assert.equal(result.measuredAt, cached.measuredAt);
    } finally {
      sb.cleanup();
    }
  });

  it('[P0] AC: cache miss (no file) returns hardcoded with source=hardcoded and correct device key', () => {
    const sb = makeSandboxProjectRoot();
    try {
      const result = resolvePerWorkerMemGB({ device: 'coreml', dtype: 'fp16', model: 'jina-code-v2' }, sb.root);
      assert.equal(result.source, 'hardcoded');
      assert.equal(result.value, WORKER_MEMORY_ESTIMATE_GB.coreml);
      assert.equal(result.measuredAt, undefined);
    } finally {
      sb.cleanup();
    }
  });

  it('[P0] AC: cache fingerprint mismatch (different model) falls back to hardcoded', () => {
    const sb = makeSandboxProjectRoot();
    try {
      // Cache was written for model A; we query for model B
      writeCalibrationCache(sb.root, { device: 'coreml', dtype: 'fp16', model: 'jina-v1' });
      const result = resolvePerWorkerMemGB({ device: 'coreml', dtype: 'fp16', model: 'jina-v2' }, sb.root);
      assert.equal(result.source, 'hardcoded');
      assert.equal(result.value, WORKER_MEMORY_ESTIMATE_GB.coreml);
    } finally {
      sb.cleanup();
  ...
  it('[P1] AC: hardcoded path picks the correct key for each device', () => {
    const sb = makeSandboxProjectRoot();
    try {
      for (const device of ['coreml', 'cuda', 'rocm', 'directml', 'cpu']) {
        const result = resolvePerWorkerMemGB({ device, dtype: 'fp16', model: 'm' }, sb.root);
        assert.equal(result.source, 'hardcoded');
        assert.equal(result.value, WORKER_MEMORY_ESTIMATE_GB[device]);
      }
    } finally {
      sb.cleanup();
    }
  ...
  it('[P1] AC: unknown device falls back to WORKER_MEMORY_ESTIMATE_GB.cpu', () => {
    const sb = makeSandboxProjectRoot();
    try {
      const result = resolvePerWorkerMemGB({ device: 'some-future-npu', dtype: 'fp16', model: 'm' }, sb.root);
      assert.equal(result.source, 'hardcoded');
      assert.equal(result.value, WORKER_MEMORY_ESTIMATE_GB.cpu);
    } finally {
      sb.cleanup();
    }
  });

  it('[P1] AC: corrupt cache JSON is treated as cache miss', () => {
    const sb = makeSandboxProjectRoot();
    try {
      fs.writeFileSync(path.join(sb.root, '.isdlc', CACHE_FILENAME), '{ not: valid json', 'utf8');
      const result = resolvePerWorkerMemGB({ device: 'cpu', dtype: 'q8', model: 'm' }, sb.root);
      assert.equal(result.source, 'hardcoded');
      assert.equal(result.value, WORKER_MEMORY_ESTIMATE_GB.cpu);
    } finally {
      sb.cleanup();
  ...
  it('[P1] AC: cache with implausible (non-finite) perWorkerMemGB falls back to hardcoded', () => {
    const sb = makeSandboxProjectRoot();
    try {
      writeCalibrationCache(sb.root, { device: 'cpu', dtype: 'q8', model: 'm' }, { perWorkerMemGB: 0 });
      const result = resolvePerWorkerMemGB({ device: 'cpu', dtype: 'q8', model: 'm' }, sb.root);
      assert.equal(result.source, 'hardcoded');
      assert.equal(result.value, WORKER_MEMORY_ESTIMATE_GB.cpu);
    } finally {
...
describe('REQ-GH-239 FR-003/FR-009/NFR-001: computeAutoParallelism uses resolved per-worker value', () => {

  // Use a DI hook to simulate the calibration resolver deterministically.
  function fakeResolver(value, source = 'calibrated') {
    return () => ({ source, value, measuredAt: '2026-04-11T05:30:00.000Z' });
  }

  it('[P0] Given a calibrated perWorkerMemGB=2.5GB on a 24GB CoreML mac, auto-parallelism uses 2.5 (not hardcoded 6)', () => {
    // 24GB total, reserved = max(8, 24*0.3=7.2) = 8, available = 16
    // With hardcoded 6: maxByMemory = floor(16/6) = 2
    // With calibrated 2.5: maxByMemory = floor(16/2.5) = 6, capped by PARALLELISM_HARD_CAP (4)
    const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
    const result = resolveConfig({}, {}, env, {
      projectRoot: '/nonexistent',
      _resolvePerWorkerMemGB: fakeResolver(2.5),
    });
    assert.equal(result.parallelism, PARALLELISM_HARD_CAP);
  });

  it('[P0] Given no calibration (hardcoded fallback), auto-parallelism matches prior hardcoded behavior', () => {
    // 24GB CoreML, perWorker=6 hardcoded -> maxByMemory = floor(16/6) = 2
    const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
    const result = resolveConfig({}, {}, env, {
      projectRoot: '/nonexistent',
      _resolvePerWorkerMemGB: fakeResolver(WORKER_MEMORY_ESTIMATE_GB.coreml, 'hardcoded'),
    });
    assert.equal(result.parallelism, 2);
  });

  it('[P0] FR-009: user max_memory_gb caps total memory even when calibrated perWorkerMemGB would allow more', () => {
    // 64GB total, user cap = 16GB -> effectiveMemGB = 16
    // reserved = max(8, 16*0.3=4.8) = 8, available = 8
    // With calibrated 2.0: maxByMemory = floor(8/2) = 4
    // maxByCpu = 11, PARALLELISM_HARD_CAP = 4 -> parallelism = 4
    const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 64 * GB, cpuCount: 12 });
    const result = resolveConfig({ max_memory_gb: 16 }, {}, env, {
      projectRoot: '/nonexistent',
      _resolvePerWorkerMemGB: fakeResolver(2.0),
    });
    assert.equal(result.parallelism, 4);
    assert.equal(result.max_memory_gb, 16);
  });

  it('[P0] FR-009: explicit parallelism override is respected regardless of calibration', () => {
    const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
    const result = resolveConfig({ parallelism: 1 }, {}, env, {
      projectRoot: '/nonexistent',
      _resolvePerWorkerMemGB: fakeResolver(2.0),
  ...
  it('[P1] NFR-001: tight max_memory_gb with calibrated perWorkerMemGB still respects memory ceiling', () => {
    // 24GB total, user cap = 10GB -> effectiveMemGB = 10
    // reserved = max(8, 10*0.3=3) = 8, available = 2
    // Calibrated perWorker = 1.5 -> maxByMemory = floor(2/1.5) = 1
    const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 8 });
    const result = resolveConfig({ max_memory_gb: 10 }, {}, env, {
      projectRoot: '/nonexistent',
      _resolvePerWorkerMemGB: fakeResolver(1.5),
    });
    assert.equal(result.parallelism, 1);
  });

  it('[P1] resolveConfig forwards projectRoot and device/dtype/model into the calibration resolver', () => {
    const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
    const seen = [];
    const spyResolver = (config, projectRoot) => {
      seen.push({ config, projectRoot });
      return { source: 'hardcoded', value: WORKER_MEMORY_ESTIMATE_GB.coreml };
    };
    resolveConfig({ model: 'jina-code-v2' }, {}, env, {
      projectRoot: '/some/project/root',
      _resolvePerWorkerMemGB: spyResolver,
    });
    assert.equal(seen.length, 1);
    assert.equal(seen[0].projectRoot, '/some/project/root');
    assert.equal(seen[0].config.device, 'coreml');
    assert.equal(seen[0].config.dtype, 'fp16');
    assert.equal(seen[0].config.model, 'jina-code-v2');
  });

  it('[P1] resolveConfig with a real sandbox calibration cache picks up the calibrated value end-to-end', () => {
    const sb = makeSandboxProjectRoot();
    try {
      // macOS ARM auto -> coreml/fp16. Write a calibration entry for that fingerprint.
      writeCalibrationCache(sb.root, { device: 'coreml', dtype: 'fp16', model: 'jina-code-v2' }, {
        perWorkerMemGB: 2.0,
      });
      // 24GB CoreML: available = 16, perWorker = 2 -> maxByMemory = 8 -> capped at PARALLELISM_HARD_CAP (4)
      const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
      const result = resolveConfig({ model: 'jina-code-v2' }, {}, env, { projectRoot: sb.root });
      assert.equal(result.parallelism, PARALLELISM_HARD_CAP);
    } finally {
      sb.cleanup();
    }
  ...
  it('[P1] resolveConfig with no calibration cache falls back to hardcoded (regression: existing parallelism math unchanged)', () => {
    const sb = makeSandboxProjectRoot();
    try {
      // No cache file written. 24GB CoreML hardcoded=6 -> maxByMemory = floor(16/6) = 2
      const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
      const result = resolveConfig({}, {}, env, { projectRoot: sb.root });
      assert.equal(result.parallelism, 2);
    } finally {
      sb.cleanup();
    }
...
// ===========================================================================
// REQ-GH-248 FR-006: Workload-aware auto parallelism
// ===========================================================================

describe('REQ-GH-248 FR-006: workload-aware autoParallelism', () => {
  // Shared fake resolver returning a fixed perWorkerMemGB
  function fakeResolver(value, source = 'calibrated') {
    return () => ({ source, value, measuredAt: '2026-04-11T05:30:00.000Z' });
  }

  it(
    '[P0] REQ-GH-248 FR-006 AC-006-01 TC-006-01: Given tiny workload (8 chunks, batch_size=32) on a 24GB Mac with perWorker=3, When resolveConfig runs, Then parallelism = 1 (workload floor = ceil(8/32/2) = 1)',
    () => {
      const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
      const result = resolveConfig({ batch_size: 32 }, {}, env, {
        projectRoot: '/nonexistent',
        workloadSize: 8,
        _resolvePerWorkerMemGB: fakeResolver(3),
      });
      assert.equal(result.parallelism, 1);
    }
  );
  ...
  it(
    '[P0] REQ-GH-248 FR-006 AC-006-02 TC-006-02: Given workload of 512 chunks on a 24GB Mac with perWorker=6, When resolveConfig runs, Then parallelism = 2 (memory ceiling binds; workload floor = 8 is not)',
    () => {
      const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
      const result = resolveConfig({ batch_size: 32 }, {}, env, {
        projectRoot: '/nonexistent',
        workloadSize: 512,
        _resolvePerWorkerMemGB: fakeResolver(6),
      });
      // maxByMemory = floor(16/6) = 2, workloadFloor = ceil(512/32/2) = 8
      // min(maxByCpu=9, maxByMem=2, hardCap=4, floor=8) = 2
      assert.equal(result.parallelism, 2);
    }
  );

  it(
    '[P0] REQ-GH-248 FR-006 AC-006-03 TC-006-03: Given parallelism: 8 (explicit) and workloadSize: 8, When resolveConfig runs, Then parallelism = 8 (explicit respected) AND a warning recommends "auto"',
    () => {
      const env = mockEnv({ platform: 'linux', totalMem: 16 * GB, cpuCount: 8 });
      const result = resolveConfig({ parallelism: 8, batch_size: 32 }, {}, env, {
        projectRoot: '/nonexistent',
        workloadSize: 8,
        _resolvePerWorkerMemGB: fakeResolver(2),
      });
      assert.equal(result.parallelism, 8, 'explicit value must be respected');
      const relevant = result.warnings.filter((w) => /workload|parallelism/i.test(w));
      assert.ok(relevant.length >= 1, `expected at least one workload warning, got ${JSON.stringify(result.warnings)}`);
    }
  );

  it(
    '[P0] REQ-GH-248 FR-006 AC-006-04 TC-006-04: computeWorkloadFloor boundary table (batchSize=32, MIN_BATCHES_PER_WORKER=2)',
    () => {
      const table = [
        [0, 0],
        [1, 1],
      ...
      for (const [workload, expected] of table) {
        const got = computeWorkloadFloor(workload, 32);
        assert.equal(got, expected, `workload ${workload}: expected ${expected}, got ${got}`);
      }
    }
  );

  it(
    '[P1] REQ-GH-248 FR-006: Given no workloadSize provided, When resolveConfig runs, Then parallelism falls back to memory/cpu cap only (no workload floor)',
    () => {
      // 24GB Mac / CoreML / hardcoded=6 → maxByMemory = 2
      // Without workloadSize, result is 2 (same as old behavior)
      const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
      const result = resolveConfig({}, {}, env, {
        projectRoot: '/nonexistent',
        _resolvePerWorkerMemGB: fakeResolver(6, 'hardcoded'),
      });
      assert.equal(result.parallelism, 2);
    }
  );
  ...
  it(
    '[P1] REQ-GH-248 FR-006: Given workloadSize is 0, When resolveConfig runs, Then workload floor is ignored (pool never sizes to zero in auto path)',
    () => {
      const env = mockEnv({ platform: 'darwin', arch: 'arm64', totalMem: 24 * GB, cpuCount: 10 });
      const result = resolveConfig({}, {}, env, {
        projectRoot: '/nonexistent',
      ...
      // With perWorker=2, maxByMem=8, cpu=9, cap=4 → 4
      assert.equal(result.parallelism, PARALLELISM_HARD_CAP);
    }
  );
});
...
// ===========================================================================
// REQ-GH-248 FR-008: computeEffectiveParallelism helper
// ===========================================================================

describe('REQ-GH-248 FR-008: computeEffectiveParallelism helper (TC-008-01)', () => {
  it('[P0] AC-008-01: min(memoryCap, cpuCap, hardCap) when no workloadFloor', () => {
    // 6 representative tuples covering each binding cap
    const table = [
      // tight memory
      { memoryCap: 2, cpuCap: 9, hardCap: 4, workloadFloor: 0, expected: 2 },
      // tight cpu
      { memoryCap: 8, cpuCap: 1, hardCap: 4, workloadFloor: 0, expected: 1 },
      // tight hardCap
      { memoryCap: 8, cpuCap: 9, hardCap: 4, workloadFloor: 0, expected: 4 },
      // tight workloadFloor
      { memoryCap: 8, cpuCap: 9, hardCap: 4, workloadFloor: 2, expected: 2 },
      // all slack → hardCap binds
      { memoryCap: 16, cpuCap: 16, hardCap: 4, workloadFloor: 0, expected: 4 },
      // all slack with workloadFloor smaller than hardCap
      { memoryCap: 16, cpuCap: 16, hardCap: 4, workloadFloor: 3, expected: 3 },
    ];
    for (const row of table) {
      const got = computeEffectiveParallelism(row);
      assert.equal(got, row.expected, `input ${JSON.stringify(row)}: got ${got}`);
    }
  });

  it('[P0] AC-008-01: floor is always >= 1 even when caps are 0', () => {
    // Zero caps should clamp to 1 (pool never has zero workers in auto path)
    assert.equal(computeEffectiveParallelism({ memoryCap: 0, cpuCap: 0, hardCap: 4 }), 1);
  });

  it('[P0] AC-008-02: MIN_BATCHES_PER_WORKER is exported and equals 2', () => {
    assert.equal(MIN_BATCHES_PER_WORKER, 2);
  });
});
