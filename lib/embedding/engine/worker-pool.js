...
// REQ-GH-248 FR-008 / AC-008-02: single source of truth for parallelism math.
// Import the helpers from device-detector.js instead of duplicating them.
//
// NOTE on circular import: memory-calibrator.js imports this module, and
// device-detector.js imports memory-calibrator.js. Importing the *values*
// (PARALLELISM_HARD_CAP, SYSTEM_RESERVED_GB) at the module top level would
// hit a TDZ error because worker-pool.js is evaluated before device-detector
// has finished initialising its constants. The functions
// (`computeEffectiveParallelism`, `computeWorkloadFloor`) are hoisted and
// safe to import here; for the constants we declare local aliases and let
// `resolvePoolSize()` read the canonical values at call time (which is
// well after all three modules have finished evaluating).
 ...
 *  Intentionally lower than the device-specific CoreML constant — the worker
 *  pool path can be called without a device hint, so the 3 GB floor avoids
 *  blowing out 24 GB boxes that spawn CPU pools. */
const GENERIC_WORKER_MEM_GB = 3;

/**
 * Resolve effective pool size from the poolSize option.
 * 'auto' → min(cpus - 1, memory-based cap, workload floor, hard cap). (AC-001-04)
 * Explicit number is clamped to >= 1.
 *
 * REQ-GH-248 FR-006 / FR-008: workload-aware resolution via the shared
 * `computeEffectiveParallelism` helper. When `workloadSize` and `batchSize`
 * are known (and poolSize === 'auto'), the workload floor
 * `ceil(workloadSize / batchSize / MIN_BATCHES_PER_WORKER)` is an additional
 * upper bound — the pool will not spawn more workers than there are
 * batches to keep them busy.
 *
 ...
 * @param {number} [workloadSize] - REQ-GH-248 FR-006: number of texts to embed (ignored when poolSize is explicit)
 * @param {number} [batchSize=32] - texts per batch (required when workloadSize is present)
 * @returns {number}
 */
export function resolvePoolSize(poolSize, cpuCountFn, totalMemFn, perWorkerMemGB, workloadSize, batchSize = 32) {
  // Read the constants lazily at call time so the circular import
  // (worker-pool → device-detector → memory-calibrator → worker-pool)
  // is fully evaluated by the time we dereference them.
  const POOL_HARD_CAP = deviceDetector.PARALLELISM_HARD_CAP;
  const POOL_RESERVED_GB = deviceDetector.SYSTEM_RESERVED_GB;
  ...
  if (poolSize === 'auto' || poolSize === undefined || poolSize === null) {
    ...
    const workloadFloor = Number.isFinite(workloadSize) && workloadSize > 0
      ? computeWorkloadFloor(workloadSize, batchSize)
      : 0;

    return computeEffectiveParallelism({
      memoryCap: maxByMemory,
      cpuCap: maxByCpu,
      hardCap: POOL_HARD_CAP,
      workloadFloor,
    });
  }
...
export function createWorkerPool(workerPath, options = {}) {
  const {
    ...
    _WorkerClass = null,
    perWorkerMemGB = null,
    // REQ-GH-248 FR-006: workload-aware pool sizing. When poolSize is
    // 'auto' and workloadSize is known, resolvePoolSize applies a
    // workload floor so small workloads do not spawn full pools.
    workloadSize = undefined,
    batchSize = 32,
  } = options;

  const effectiveSize = resolvePoolSize(
    poolSizeOpt,
    _cpuCountFn,
    _totalMemFn,
    perWorkerMemGB,
