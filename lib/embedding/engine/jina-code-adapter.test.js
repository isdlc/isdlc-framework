...
describe('FR-001: Jina Code Adapter (jina-code-adapter)', () => {
  ...
  // ══════════════════════════════════════════════════════════════════
  // REQ-GH-239 FR-002: Unified adapter.embed() signature
  // Phase 05 scaffolds (test.skip) — implementation lands in Phase 06
  // ══════════════════════════════════════════════════════════════════

  describe('REQ-GH-239 FR-002: Unified adapter.embed(texts, options) signature', () => {

    it('[P0] REQ-GH-239 FR-002: Given an in-process adapter (parallelism=1), When adapter.embed(texts, { batchSize, onProgress, signal }) is called, Then it accepts the options object and returns a Promise<Float32Array[]>', async () => {
      const { adapter } = await createTestAdapter({ parallelism: 1 });
      const progressCalls = [];
      ...
      assert.ok(Array.isArray(result), 'result should be an array');
      assert.equal(result.length, 3, 'result should have 3 vectors');
      for (let i = 0; i < 3; i++) {
        assert.ok(result[i] instanceof Float32Array, `result[${i}] should be Float32Array`);
        assert.equal(result[i].length, 768, `result[${i}] should have 768 dimensions`);
      }
      // In-process path synthesized progress events
      assert.ok(progressCalls.length > 0, 'onProgress should have been called at least once');
    });

    it('[P0] REQ-GH-239 FR-002: Given a pooled adapter (parallelism>1), When adapter.embed(texts, { batchSize, onProgress, signal }) is called, Then it accepts the options object and returns a Promise<Float32Array[]>', async () => {
      const { createPool, poolLog } = createMockWorkerPool();
      const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
      const { pipelineFn } = createMockPipeline();
      const adapter = await createJinaCodeAdapter({
      ...
      assert.ok(Array.isArray(result), 'result should be an array');
      assert.equal(result.length, 3, 'result should have 3 vectors');
      const embedCalls = poolLog.filter(e => e.method === 'embed');
      assert.equal(embedCalls.length, 1, 'pool.embed should be called exactly once');
      assert.deepEqual(embedCalls[0].texts, ['a', 'b', 'c'], 'texts forwarded verbatim');
      assert.equal(embedCalls[0].batchSize, 32, 'batchSize forwarded');
      assert.equal(embedCalls[0].opts.onProgress, progressFn, 'onProgress forwarded');
      assert.equal(embedCalls[0].opts.signal, signal, 'signal forwarded');
    });

    it('[P0] REQ-GH-239 FR-002: Given a pooled adapter, When adapter.embed(texts, options) is called with a large text array, Then pool.embed is invoked EXACTLY ONCE (no outer batch loop)', async () => {
      const { createPool, poolLog } = createMockWorkerPool();
      const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
      const { pipelineFn } = createMockPipeline();
      const adapter = await createJinaCodeAdapter({
      ...
      // 500 texts — way more than any single batch
      const texts = Array.from({ length: 500 }, (_, i) => `chunk-${i}`);
      await adapter.embed(texts, { batchSize: 32 });
      const embedCalls = poolLog.filter(e => e.method === 'embed');
      assert.equal(embedCalls.length, 1, 'pool.embed should be called exactly once, no outer loop');
      assert.equal(embedCalls[0].texts.length, 500, 'all 500 texts sent in single call');
      assert.deepEqual(embedCalls[0].texts, texts, 'full text array forwarded unchanged');
    });

    it('[P0] REQ-GH-239 FR-002: Given an in-process adapter, When adapter.embed(texts, { onProgress }) is called with 6 texts and batchSize=2, Then onProgress fires with the unified FR-005 shape (processed/total/chunks_per_sec/eta_seconds/active_workers)', async () => {
      const { adapter } = await createTestAdapter({ parallelism: 1 });
      const progressCalls = [];
      await adapter.embed(['a', 'b', 'c', 'd', 'e', 'f'], {
        batchSize: 2,
        onProgress: (p) => progressCalls.push(p),
      });
      assert.ok(progressCalls.length >= 3, `expected >=3 progress events, got ${progressCalls.length}`);
      // Every progress event must have the unified FR-005 shape.
      for (const p of progressCalls) {
        assert.ok('processed' in p, 'progress should include processed');
        assert.ok('total' in p, 'progress should include total');
        assert.ok('chunks_per_sec' in p, 'progress should include chunks_per_sec');
        assert.ok('eta_seconds' in p, 'progress should include eta_seconds');
        assert.ok('active_workers' in p, 'progress should include active_workers');
        assert.equal(p.total, 6, 'total should be 6');
        assert.equal(p.active_workers, 1, 'in-process reports active_workers=1');
        assert.ok(typeof p.chunks_per_sec === 'number', 'chunks_per_sec should be number');
        assert.ok(typeof p.eta_seconds === 'number', 'eta_seconds should be number');
      }
      // The terminal event should report processed===total and eta_seconds===0.
      const last = progressCalls[progressCalls.length - 1];
      assert.equal(last.processed, 6, 'final progress.processed === total');
      assert.equal(last.eta_seconds, 0, 'final progress.eta_seconds === 0');
    });

    it('[P1] REQ-GH-239 FR-002: Given options.signal is already aborted, When adapter.embed is called, Then it rejects with an AbortError and does NOT dispatch any batches to the pool', async () => {
      // Pooled path
      const { createPool, poolLog } = createMockWorkerPool();
      const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
      const { pipelineFn } = createMockPipeline();
      ...
      controller.abort();
      await assert.rejects(
        () => adapter.embed(['a', 'b'], { signal: controller.signal }),
        (err) => {
          assert.ok(err instanceof Error, 'should be an Error');
          const isAbort = err.name === 'AbortError' ||
            /cancel|abort/i.test(err.message);
          assert.ok(isAbort, `error should indicate abort, got: ${err.name}/${err.message}`);
          return true;
        }
      );
      const embedCalls = poolLog.filter(e => e.method === 'embed');
      assert.equal(embedCalls.length, 0, 'pool.embed should NOT be dispatched when pre-aborted');

      // In-process path: pre-aborted signal also rejects without embedding.
      const { adapter: ipAdapter, callLog: ipCallLog } = await createTestAdapter({ parallelism: 1 });
      const controller2 = new AbortController();
      controller2.abort();
      await assert.rejects(
        () => ipAdapter.embed(['a', 'b'], { signal: controller2.signal }),
        (err) => {
          const isAbort = err.name === 'AbortError' ||
            /cancel|abort/i.test(err.message);
          assert.ok(isAbort, 'in-process should also reject with abort');
          return true;
        }
      );
      // Pipeline may have been initialised by healthCheck/ensureExtractor
      // but no actual extractor calls should have been made with text content.
      const extractorInvocations = ipCallLog.filter(e => 'texts' in e);
      assert.equal(extractorInvocations.length, 0, 'no extractor invocations on pre-abort');
    });

    it('[P1] REQ-GH-239 FR-002: Given options is omitted (legacy 1-arg call), When adapter.embed(texts) is called, Then it still works with default batchSize and no progress callback (backwards compatible)', async () => {
      // In-process path
      const { adapter: ipAdapter } = await createTestAdapter({ parallelism: 1 });
      const ipResult = await ipAdapter.embed(['x', 'y']);
      assert.ok(Array.isArray(ipResult));
      assert.equal(ipResult.length, 2, 'in-process returns 2 vectors');
      assert.ok(ipResult[0] instanceof Float32Array, 'in-process returns Float32Array');

      // Pooled path
      const { createPool, poolLog } = createMockWorkerPool();
      const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
      const { pipelineFn } = createMockPipeline();
      const pooledAdapter = await createJinaCodeAdapter({
        _pipelineFactory: pipelineFn,
        _createWorkerPool: createPool,
        parallelism: 2,
      });
      const pooledResult = await pooledAdapter.embed(['x', 'y']);
      assert.ok(Array.isArray(pooledResult));
      assert.equal(pooledResult.length, 2, 'pooled returns 2 vectors');
      const embedCall = poolLog.find(e => e.method === 'embed');
      assert.ok(embedCall, 'pool.embed called in legacy 1-arg form');
      assert.equal(embedCall.batchSize, 32, 'pooled legacy defaults to resolved batch_size=32');
    });

    it('[P1] REQ-GH-239 FR-002: Given a pooled adapter, When adapter.embed is called, Then options.batchSize is forwarded verbatim to pool.embed and not re-computed or defaulted', async () => {
      const { createPool, poolLog } = createMockWorkerPool();
      const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
      const { pipelineFn } = createMockPipeline();
      // Adapter constructed with batch_size: 32 (resolved default)
      const adapter = await createJinaCodeAdapter({
        _pipelineFactory: pipelineFn,
        _createWorkerPool: createPool,
        parallelism: 2,
      });
      assert.equal(adapter._resolved.batch_size, 32, 'construction-time batch_size is 32');
      // Call-time override: batchSize: 16 should win
      await adapter.embed(['a', 'b', 'c'], { batchSize: 16 });
      const embedCall = poolLog.find(e => e.method === 'embed');
      assert.ok(embedCall, 'pool.embed should have been called');
      assert.equal(embedCall.batchSize, 16, 'call-time batchSize=16 wins over construction-time 32');
    });

  });

  // ══════════════════════════════════════════════════════════════════
  // REQ-GH-248 FR-007: Adapter calibrated value pass-through
  // ══════════════════════════════════════════════════════════════════

  describe('REQ-GH-248 FR-007: calibrated perWorkerMemGB pass-through (TC-007-01..04)', () => {
    const darwinArm = { platform: 'darwin', arch: 'arm64', pathExists: () => false, totalMem: 24 * 1024 ** 3, cpuCount: 10 };
    // Shared fake resolver that yields a specific calibrated value
    function fakeResolver(value) {
      return () => ({ source: 'calibrated', value, measuredAt: '2026-04-11T05:30:00.000Z' });
    }

    it(
      '[P0] REQ-GH-248 FR-007 AC-007-01 TC-007-01: Given a calibrated perWorkerMemGB=3.5, When the adapter builds its pool, Then createPool.options.perWorkerMemGB === 3.5 (NOT WORKER_MEMORY_ESTIMATE_GB.coreml)',
      async () => {
        const { createPool, poolLog } = createMockWorkerPool();
        const { createJinaCodeAdapter, JINA_CODE_DIMENSIONS } = await import('./jina-code-adapter.js');
        const { pipelineFn } = createMockPipeline();
        // Thread a fake perWorkerMemGB resolver into the device-detector
        // by leveraging the existing _platformEnv DI and a sandboxed config.
        // The adapter's internal resolveConfig call will pick up the
        // fakeResolver via the opts argument only if we can inject it,
        // which today requires going through resolveConfig directly or
        // by making the adapter observe resolved.perWorkerMemGB. Since
        // the adapter already propagates resolved.perWorkerMemGB to
        // createPool, we simply verify the end-to-end contract: if the
        // resolved config carries a calibrated value, it reaches the pool.
        //
        // Use a real 24GB CoreML sandbox with no cache (hardcoded path)
        // and then override via the adapter's config.max_memory_gb +
        // parallelism setup. Simpler: use a sandbox projectRoot with a
        // calibration cache file, matching REQ-GH-239 end-to-end tests.
        const fs = await import('node:fs');
        const os = await import('node:os');
        const pathMod = await import('node:path');
        const { computeFingerprint, CACHE_FILENAME } = await import('./memory-calibrator.js');
        const sandboxDir = fs.mkdtempSync(pathMod.join(os.tmpdir(), 'adapter-calib-'));
        fs.mkdirSync(pathMod.join(sandboxDir, '.isdlc'), { recursive: true });
        try {
          // Write a calibration cache for the auto-resolved (coreml, fp16)
          // pair. NOTE: the adapter does NOT currently forward `model` into
          // resolveConfig → _resolvePerWorkerMemGB, so the fingerprint is
          // computed with `model: undefined`. Match that here.
          const cfg = { device: 'coreml', dtype: 'fp16' };
          const fp = computeFingerprint(cfg);
          fs.writeFileSync(
            pathMod.join(sandboxDir, '.isdlc', CACHE_FILENAME),
            JSON.stringify({
              perWorkerMemGB: 3.5,
              baselineMemGB: 0.3,
              peakMemGB: 3.0,
          ...
          // Change cwd to the sandbox so resolveConfig picks up the cache
          ...
          try {
            const adapter = await createJinaCodeAdapter({
              _pipelineFactory: pipelineFn,
              _createWorkerPool: createPool,
              _platformEnv: darwinArm,
              parallelism: 2,
            });
            assert.ok(adapter);
            assert.equal(adapter._resolved.perWorkerMemGB, 3.5);
            const createCall = poolLog.find((e) => e.method === 'createPool');
            assert.ok(createCall);
            assert.equal(createCall.options.perWorkerMemGB, 3.5);
          } finally {
    ...
    it(
      '[P0] REQ-GH-248 FR-007 AC-007-03 TC-007-03: Given no calibration cache, When the adapter builds its pool, Then createPool.options.perWorkerMemGB falls back to WORKER_MEMORY_ESTIMATE_GB[device] (fail-safe default, Article X)',
      async () => {
        const { createPool, poolLog } = createMockWorkerPool();
        const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
        const { WORKER_MEMORY_ESTIMATE_GB } = await import('./device-detector.js');
        const { pipelineFn } = createMockPipeline();
        // darwin arm64 → coreml. No cache → resolveConfig returns the
        // hardcoded fallback value (6 GB for coreml).
        const fs = await import('node:fs');
        const os = await import('node:os');
        const pathMod = await import('node:path');
        const sandboxDir = fs.mkdtempSync(pathMod.join(os.tmpdir(), 'adapter-nocache-'));
        fs.mkdirSync(pathMod.join(sandboxDir, '.isdlc'), { recursive: true });
        try {
          const origCwd = process.cwd();
          process.chdir(sandboxDir);
          try {
            ...
            assert.ok(adapter);
            assert.equal(adapter._resolved.perWorkerMemGB, WORKER_MEMORY_ESTIMATE_GB.coreml);
            const createCall = poolLog.find((e) => e.method === 'createPool');
            assert.ok(createCall);
            assert.equal(createCall.options.perWorkerMemGB, WORKER_MEMORY_ESTIMATE_GB.coreml);
          } finally {
            process.chdir(origCwd);
          }
        } finally {
    ...
    it(
      '[P1] REQ-GH-248 FR-007 TC-007-04: regression guard — createPool receives a perWorkerMemGB option (not undefined)',
      async () => {
        const { createPool, poolLog } = createMockWorkerPool();
        const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
        const { pipelineFn } = createMockPipeline();
        ...
        assert.ok(createCall);
        assert.ok(
          Number.isFinite(createCall.options.perWorkerMemGB) && createCall.options.perWorkerMemGB > 0,
          `perWorkerMemGB must be set and positive, got ${createCall.options.perWorkerMemGB}`
        );
      }
    );

    it(
      '[P1] REQ-GH-248 FR-006 + FR-007: workloadSize is threaded through createJinaCodeAdapter → resolveConfig → createPool.options.workloadSize',
      async () => {
        const { createPool, poolLog } = createMockWorkerPool();
        const { createJinaCodeAdapter } = await import('./jina-code-adapter.js');
        const { pipelineFn } = createMockPipeline();
        ...
        const createCall = poolLog.find((e) => e.method === 'createPool');
        assert.ok(createCall);
        assert.equal(createCall.options.workloadSize, 256);
      }
    );
  });
