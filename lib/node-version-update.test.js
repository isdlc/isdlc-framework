...
describe('REQ-004: constitution.md', () => {
  ...
  // TC-022: Constitution version is 1.3.0
  it('TC-022: constitution version is "1.4.0" (AC-14, P0)', () => {
    assert.ok(constContent.includes('**Version**: 1.4.0'),
      'constitution.md does not have version 1.4.0');
  });
  ...
  it('TC-025: only Article XII, version header, and amendment log changed (AC-15, P1)', () => {
    ...
    assert.ok(constLines[3].includes('**Version**: 1.4.0'),
...
describe('REQ-005: README.md', () => {
  ...
  it('TC-028: README system requirements shows "Node.js 20+" (AC-17, P0)', () => {
    assert.ok(readmeContent.includes('**Node.js**') && readmeContent.includes('| 20+'),
      'README does not contain "**Node.js**" with "20+" in system requirements table');
  });
});
...
describe('REQ-006: state.json', () => {
  ...
  it('TC-029: state.json tech_stack.runtime reads "node-20+" (AC-18, P1)', () => {
    ...
    if (!state.project) {
      // state.json exists but has no project key; skip
      return;
    }
...
describe('NFR-004: documentation consistency', () => {
  // TC-036: Discovery report shows Node.js version info
  it('TC-036: project-discovery-report shows Node.js "20/22/24" (NFR-004, P2)', () => {
    const content = readFileSync(DISCOVERY_REPORT, 'utf8');
    assert.ok(content.includes('20/22/24'),
      'project-discovery-report.md does not contain "20/22/24"');
  });

  // TC-037: Discovery report shows CI tests all three Node versions
  it('TC-037: project-discovery-report shows "CI tests all three" (NFR-004, P2)', () => {
    const content = readFileSync(DISCOVERY_REPORT, 'utf8');
    assert.ok(content.includes('CI tests all three'),
      'project-discovery-report.md does not contain "CI tests all three"');
  });
